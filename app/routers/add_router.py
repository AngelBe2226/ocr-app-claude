...
def add_options(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    # Tiendas ya usadas (para autocompletar el campo Tienda/Comercio).
    stores = sorted({(t.store or "").strip() for t in db.query(Transaction).filter(Transaction.user_id == user.id).all()
                     if (t.store or "").strip()})
    return JSONResponse({
...
def add_expense(
    ...
    _add_transaction(db, user, "expense", profile, category, account_id, amount, date, note,
                     lat, lng, place_name, attachment, subcategory=subcategory, store=store)
