...
from app.finance import fmt_eur, hash_color, net_worth_eur, ring_dash
from app.models import Account, Budget, Goal, Loan, Transaction, User
...
def overview(request: Request, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    for pid in PROFILE_IDS:
        budgets = db.query(Budget).filter(Budget.user_id == user.id, Budget.profile == pid).all()
